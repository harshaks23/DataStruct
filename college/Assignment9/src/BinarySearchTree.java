...
public class BinarySearchTree<E> extends AbstractSet<E> 
{
    ...
    // For Adding Elements Recursively
    ...
    private boolean addRec(Entry<E> current, E element) {
        // System.out.println("In addAux: " + curr.element);
        int comp = ((Comparable)element).compareTo(current.element);
        if (comp == 0) {
            return false;
        }
        else if (comp < 0) {
            if (current.left == null) {
                current.left = new Entry<E>(element, current);
                size++;
                return true;
            }
            else {
                return addRec(current.left, element);
            }
        }
        else {
            if (current.right == null) {
                current.right = new Entry<E>(element, current);
                size++;
                return true;
            }
            else {
                return addRec(current.right, element);
            }
        }
    }
    ...
    public void inorder_iterative() { inorder_p_iterative(root); }
    private void inorder_p_iterative(Entry<E> t)  {
 	   Entry<E>curr=t;
 		 Stack<Entry<E>>temp=new Stack<Entry<E>>();
 		 boolean d=false;
 		 while(!d){
 			 if(curr!=null){
 ...
 /*   public int height(){
   ...
   */ 
    public int height(){
		int n=height_cal(root);
		return n;
	}
	
	public  int height_cal(Entry<E> t){
		if(t == null){
			return -1;
		}
		else
			return 1+Math.max(height_cal(t.left), height_cal(t.right));			
	   }
