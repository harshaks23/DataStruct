...
public class CS401BST<E extends Comparable<E>>
{
    ...
    private boolean addRecursion(TreeNode<E> current, E element) {
        ...
        if (comp == 0) {
            ...
            if (current.left == null) {
                current.left = new TreeNode<E>(element, current);
                size++;
                return true;
            }
            else {
                return addRecursion(current.left, element);
            }
        }
        else {
            if (current.right == null) {
                current.right = new TreeNode<E>(element, current);
                size++;
                return true;
            }
            else {
                return addRecursion(current.right, element);
            }
        }
    }
